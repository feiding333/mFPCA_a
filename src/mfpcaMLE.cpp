...
void mfpcaMLE::updateSigmaSq(List UWUt, vec params){
    ...
    do{
        ...
        lambda = updateSigmaSqBackTracking(res, UWUt, alpha, beta, verbose); // update by back tracking
        if(verbose > 0) Rcout << "Iter = " << iter << "; lambda = " 
                              << lambda << std::endl;
        iter++;
    } while (iter < 5 && lambda > epsilon);
}
...
vec mfpcaMLE::updateSigmaSqGradient(List UWUt){
    ...
    for(int i = 0; i < totalS; i++){
        ...
        hess += 2 * dot(tmpY, PiInv * tmpY) - dot(PiInv, PiInv);
    }
...
double mfpcaMLE::updateSigmaSqBackTracking(vec res, List UWUt, 
    ...
    double stepsize = 1, expectedDesc, actualDesc;
    int iterInner = 0;
    
    // the newton decrement, in Boyd's book
    lambda = grad * grad / hess;
    expectedDesc = lambda / beta * alpha;
